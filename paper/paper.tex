...
Table~\ref{tab:alpha} summarizes scaling exponents across dimensions: $\alpha_{2\text{D}}=0.628$, $\alpha_{3\text{D}}=0.789$. The inequality $\alpha_{3\text{D}}>\alpha_{2\text{D}}$ is counterintuitive, since higher connectivity should in principle accelerate energy diffusion. We attribute this to a \emph{spectral bottleneck} mechanism, which we quantify directly by comparing the minimum linear normal-mode spacing $\Delta\omega_{\min}(L)$ (from the 3D cubic dispersion relation) against the nonlinear linewidth $\Gamma_{\text{NL}}=\kappa\langle\theta^2\rangle$ measured from simulation (Fig.~\ref{fig:bottleneck}, Table~\ref{tab:bottleneck}).
...
\subsection{The 3D Spectral Bottleneck}
The quantified ratio $\Delta\omega_{\min}/\Gamma_{\text{NL}}$ remains in the range 10--113 throughout $L=2$--$15$: the suppression of 4-wave resonances persists across the entire simulated range, with only a slow, non-monotonic decline in strength as $L$ increases. Reaching the cascade regime (ratio $\approx1$) would require substantially larger $L$, outside our current computational reach; we therefore characterize the crossover as gradual rather than sharp.
